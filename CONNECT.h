...
void OnDataSent(const wifi_tx_info_t *info, esp_now_send_status_t status) {
  ...
  // If you need to access the MAC address, you can use:
  // const uint8_t *mac_addr = info->peer_addr;
}


void OnDataRecv(const esp_now_recv_info* recv_info, const uint8_t *incomingData, int len) {
  if(DEV_ROLE == 2){
    const uint8_t* mac = recv_info->src_addr;  // Получаем MAC адрес из новой структуры
    ...
    // Если нужно использовать MAC адрес, используйте переменную mac
